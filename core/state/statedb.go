		...
		func() error {
			...
			if s.snap != nil {
				...
				if parent := s.snap.Root(); parent != root {
					...
					// Keep 16 diff layers in the memory, persistent layer is 129th.
					// - head layer is paired with HEAD state
					// - head-1 layer is paired with HEAD-1 state
					// - head-127 layer(bottom-most diff layer) is paired with HEAD-127 state
					if err := s.snaps.Cap(root, 16); err != nil {
						log.Warn("Failed to cap snapshot tree", "root", root, "layers", 16, "err", err)
					}
				}
